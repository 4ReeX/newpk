<header id="header" class="header">
   <div class="top-left">
      <div class="navbar-header">
         <div class="navbar-brand" >
            <div class="logo">
               <a href="index.php"><img src="images/new-logo.svg" alt="" class="icon-logo start"></a>
               <a href=""><img src="images/invert-affimob.svg" alt="" class="text-logo"></a>
            </div>
            <div class="mob-box">
         <span class="balance">10 565 665 <i class="fa fa-rub"></i></span>
      <div class="icon-notification"><i class="ti ti-bell"><span class="count">13</span></i></div>
      <button class="mobile chat-activator pulse" style="position: relative;"><i class="ti ti-comments"></i></button>
      <div class="avatar" onclick="goToUserPhp();"><li class="nav-item"><a data-flag="pages" href="user.php"><i class="ti ti-user"></i></a></li></div>
      </div>
         </div>
         <a class="navbar-brand hidden" href="/"><img src="images/logo2.png" alt="Logo"></a>        
      </div>
   </div>
   <div class="top-right">Статистика</div>
</header>